...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // API endpoints para AJAX (apenas para perfil misto - ambos perfis acumuláveis)
...
// Rotas KML - fora do middleware auth para permitir acesso direto, mas com verificação de permissões no controller
...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // Servir documentos pelo novo sistema de arquivos
    Route::get('/property/document/serve/{id}', [PropertyController::class, 'viewDocument'])
        ->name('property.document.serve')
        ->where('id', '[0-9]+');

    // Rota temporária para teste de KML
    Route::get('/test-kml', function() {
        return response()->file(public_path('teste.kml'), [
...
Route::middleware(['auth'])->group(function () {
    Route::get('/error/unauthorized', function () {
        return Inertia::render('Error/Unauthorized', [
            ...
            'userProfiles' => Auth::user() ? Auth::user()->profiles->pluck('slug')->toArray() : [],
        ]);
