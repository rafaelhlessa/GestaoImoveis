    ...
    @elseif(!empty($d) && $evaluation->property_type === 'rural')
        ...
        <div class="card">
            ...
            <div class="mb-10">
                ...
                @php $herds = $d['rebanho']; @endphp
                @if(is_array($herds) && array_is_list($herds))
                    @foreach($herds as $idx => $h)
                        <div class="feature-label" style="margin-top:8px;">Grupo {{ $idx+1 }}: {{ $h['especie'] ?? '-' }} @if(($h['raca'] ?? null)) - Raça: {{ $h['raca'] }} @endif</div>
                        <table class="info-table" style="margin-top:2px;">
                            ...
                            @forelse($faixas as $f)
                                @php
                                    $m = (int)($f['quantidade_machos'] ?? 0);
                                    $fem = (int)($f['quantidade_femeas'] ?? 0);
                                    $sumM += $m; $sumF += $fem;
                                @endphp
                                <tr>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4">Sem faixas etárias informadas.</td>
                                </tr>
                            @endforelse
                            <tr>
                                ...
                                <td><strong>{{ $sumM + $sumF }}</strong></td>
                            </tr>
                        </table>
                    @endforeach
                @elseif(is_array($herds))
                    <table class="info-table">
                        @if(($herds['especie'] ?? null))<tr><td>Espécie</td><td>{{ $herds['especie'] }}</td></tr>@endif
                        @if(($herds['raca'] ?? null))<tr><td>Raça</td><td>{{ $herds['raca'] }}</td></tr>@endif
                        @if(($herds['faixa_etaria'] ?? null))<tr><td>Faixa Etária</td><td>{{ $herds['faixa_etaria'] }}</td></tr>@endif
                        @if(array_key_exists('quantidade_machos', $herds))<tr><td>Machos</td><td>{{ $herds['quantidade_machos'] }}</td></tr>@endif
                        @if(array_key_exists('quantidade_femeas', $herds))<tr><td>Fêmeas</td><td>{{ $herds['quantidade_femeas'] }}</td></tr>@endif
                    </table>
                @endif
            </div>
            @endif
