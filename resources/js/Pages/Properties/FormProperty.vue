...
const addOwner = (event) => {
    ...
    if (!selectedOwner.value.id || !selectedOwner.value.percent) {
        alert.message = "Por favor, selecione um proprietário e insira o percentual.";
        alert.show = true;
        alert.type = "error";
        alert.color = "red";
        setTimeout(() => {
    ...
    if (owners.value.find((o) => o.id === selectedOwner.value.id)) {
        alert.message = "Este proprietário já foi adicionado.";
        alert.show = true;
        alert.type = "warning";
        alert.color = "yellow";
        setTimeout(() => {
    ...
    // Calcula o total utilizado e o disponível para o tipo de propriedade selecionado
    const totalPercentByType = owners.value
        .filter(owner => owner.type_ownership === selectedOwner.value.type_ownership)
        .reduce((sum, owner) => sum + parseFloat(owner.percent), 0);

    const availablePercentByType = 100 - totalPercentByType;

    // Verifica se o percentual escolhido excede o disponível
    if (enteredPercent > availablePercentByType) {
        alert.message = `O percentual disponível para "${props.typeOwners.find(type => type.id === selectedOwner.value.type_ownership)?.name}" é ${availablePercentByType}%.`;
        alert.show = true;
        alert.type = "warning";
        alert.color = "yellow";
        setTimeout(() => {
    ...
    owners.value.push({
        ...
        type_ownership: selectedOwner.value.type_ownership,
...
const saveForm = () => {
    ...
    form.owners.forEach(owner => {
        owner.cpf_cnpj = sanitizeValue(owner.cpf_cnpj);
    });
...
const addDocument = () => {
    if (!newDocument.value.file) {
        ...
        // alert("Por favor, selecione um arquivo válido antes de adicionar.");
        return;
    }
    documents.push({
...
const handleDocumentUpload = async (event) => {
    ...
    // Verificar se o arquivo é um PDF ou Word
    ...
    if (!allowedTypes.includes(file.type)) {
        ...
        // alert("Por favor, selecione um arquivo PDF ou Word (.pdf, .doc, .docx)");
                        ...
                        <div class="bg-white rounded-lg shadow p-8">
                            <form @submit.prevent="submitForm" enctype="multipart/form-data">
                                <div class="space-y-12">
                                    <div class="border-b border-gray-900/10 pb-12">
                                        ...
                                        <div class="mb-8 grid grid-cols-1 sm:grid-cols-12 gap-x-6 gap-y-8">
                                            ...
                                            <div class="sm:col-span-4 col-span-full">
                                                <label for="city"
                                                    class="block text-sm font-medium text-gray-900">Município/Estado
                                                </label>
                                                <div class="mt-2">
                                                    <input id="city" type="text" v-model="form.city"
                                                        @focus="showSuggestions = true" @blur="closeSuggestions"
                                    ...
                                    <div class="border-b border-gray-900/10 pb-12">
                                        <label for="document-file" class="block text-sm font-medium text-gray-700">Foto da Propriedade</label>
                                        <input type="file" id="document-file" @change="handleFileChange" accept="image/*" 
                                    ...
                                    <div class="border-b border-gray-900/10 pb-12">
                                        ...
                                        <div class="flex justify-between">
                                            <div>
                                                <p class="mt-1 text-sm/6 text-gray-600">Inclua os documentos da
                                                    propriedade.</p>
                                                <p v-if="form.type_property === '2'" class="mt-1 text-sm/6 text-red-600">
                                                ...
                                                <p v-if="form.type_property === '1'" class="mt-1 text-sm/6 text-red-600">*
                                            ...
                                            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                                                ...
                                                <tbody class="bg-white divide-y divide-gray-200">
                                                    <tr v-for="(document, index) in documents" :key="index" class="bg-white border-b dark:bg-gray-600 dark:border-gray-700">
                                                        ...
                                                        <td class="px-6 py-4 text-gray-900 dark:text-white">{{ new Date(document.date).toLocaleDateString('pt-BR') }}</td>
                                                        ...
                                                        <td class="px-6 py-4 text-gray-900 dark:text-white">
                                                            <button type="button" @click="removeDocument" class="bg-red-500 border border-gray-600 rounded p-1 text-sm/6 font-semibold text-gray-50 hover:bg-red-700">Excluir</button>
                                                        </td>
                                                    </tr>
                                                </tbody>
                                            </table>
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="mb-4">
                                                            ...
                                                            <div class="flex items-center">
                                                                <div class="relative w-full">
                                                                    <input type="number" id="document-name" v-model="selectedOwner.percent" max="100" min="0"
                                                ...
                                                <div class="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                                                    ...
                                                    <form @submit.prevent="addDocument">
                                                        ...
                                                        <div class="mb-4">
                                                            ...
                                                            <input type="date" id="document-name" v-model="newDocument.date"
                                                        ...
                                                        <div class="mb-4">
                                                            ...
                                                            <input type="file" id="document-file" @change="handleDocumentUpload" accept=".pdf,.doc,.docx"
                                ...
                                <div class="mt-6 flex items-center justify-end gap-x-6">
                                    ...
                                    <button @click="saveForm" class="rounded-md bg-emerald-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-emerald-500 focus-visible:outline-1 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">Salvar</button>
                                </div>
                            </form>
                        </div>
