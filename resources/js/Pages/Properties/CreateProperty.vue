...
const addOwner = (event) => {
    ...
    // Verifica se a soma dos percentuais dos proprietários excede 100%
    const totalProprietarioPercent = owners.value
        .filter(owner => props.typeOwners.find(type => type.id === owner.type_ownership)?.name === 'Proprietário')
        .reduce((sum, owner) => sum + parseFloat(owner.percent), 0);

    if (props.typeOwners.find(type => type.id === selectedOwner.value.type_ownership)?.name === 'Proprietário' && (totalProprietarioPercent + enteredPercent) > 100) {
    ...
    // Verifica se a soma dos percentuais das outras opções excede 100%
    const totalOtherPercent = owners.value
        .filter(owner => props.typeOwners.find(type => type.id === owner.type_ownership)?.name !== 'Proprietário')
        .reduce((sum, owner) => sum + parseFloat(owner.percent), 0);

    if (props.typeOwners.find(type => type.id === selectedOwner.value.type_ownership)?.name !== 'Proprietário' && (totalOtherPercent + enteredPercent) > 100) {
